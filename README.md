...
通过屏幕视觉识别（YOLO 粗识别 + 模板匹配精识别）在游戏仓库中自动检测并出售物品。
...
- **价格识别工具**：保留 EasyOCR 价格读取模块，当前主卖出流程未自动调用
- **定价算法工具**：保留对称减法算法，当前主卖出流程未自动输入 OCR 价格
- **背包页面校验**：卖出前通过固定锚点像素检测背包页面是否仍在
...
torch>=2.0.0          # YOLO/可选 GPU 加速
```

`torch` 在 Windows 上会按环境安装 CPU 或 CUDA 构建；如果需要 CUDA，请按 PyTorch 官方命令安装与本机驱动匹配的版本。
